...
protected:
   ...
   */
   ...
   public:
      ...
      pair<int,int> upLeft; 
       int dim;       //dimension
      RGBAPixel avg;   //avg color
      double var;    //color variability
...
void renderHelper(Node *curr, PNG* png);
int idealPruneHelper(int leaves, int min, int max);
int pruneSizeHelper(int tol, Node *curr);
void pruneHelper(int tol, Node *curr);
void clearHelper(Node* &current);
Node * copyHelper (const Node* newRoot);
